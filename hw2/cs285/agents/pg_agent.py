...
class PGAgent(nn.Module):
    ...
    def update(
        ...
        # Ensure all arrays have the same leading dimension of `batch_size`
        assert obs.shape[0] == actions.shape[0] == rewards.shape[0] == terminals.shape[0] == q_values.shape[0] == batch_size, "All arrays should have the same batch size"


        # batch_size = sum(len(traj_rewards) for traj_rewards in rewards)
        # obs = np.concatenate(obs)
        # actions = np.concatenate(actions)
        # rewards = np.concatenate(rewards)
        # terminals = np.concatenate(terminals)
        ...
        if self.critic is not None:
            ...
            for i in range(self.baseline_gradient_steps-1):
                self.critic.update(obs, q_values) # JL: why pass in q_values? q_values approximates the sum of 
                # future rewards from the (state, action) pair, but we want the critic to estimate the 
                # sum of future rewards from the state, instead of the (state, action) pair.
    ...
    #     if self.critic is None:
    #         # TODO: if no baseline, then what are the advantages?
    #         # If no baseline, the advantages are simply the Q-values
    #         advantages = q_values
    #     else:
    #         # TODO: run the critic and use it as a baseline
    ...
    def _discounted_return(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        for i in range(len(rewards)):
            ...
            for j in range(0, len(rewards)):
                discounted_reward += (self.gamma ** j) * rewards[j]
